"""
Keypad diagnostic — run this on the Orange Pi to verify all 12 keys are working.

    sudo systemctl stop pisonet
    cd /path/to/pisonex/server-orangepi
...
Wiring used (OPi.GPIO BCM / SoC numbering — update ROWS/COLS to match your board):
...
try:
    ...
    sys.exit("OPi.GPIO not found — install it with: pip install OPi.GPIO")
