...
from models import AdminUser, CoinTransaction, SystemLog, CoinRate, RateProfile, PC, Session as SessionModel, User, MembershipConfig, ServerConfig, CoinSchedule, ScheduledAnnouncement
...
def reset_member_password(
    ...
    if not result["success"]:
        raise HTTPException(status_code=404, detail=result["error"])
    return {"status": "reset", "username": result["username"], "temp_password": result["temp_password"]}
...
def system_health(
    ...
    """Live Raspberry Pi hardware metrics — CPU, RAM, disk, temperature, uptime."""
    if not current_user:
        raise HTTPException(status_code=401, detail="Not authenticated")
...
def _is_dhcp(iface: str) -> bool:
    # nmcli (NetworkManager — most modern Armbian/RPi OS builds)
    try:
...
def get_network_config(
    ...
    """Return the current network configuration of the Raspberry Pi."""
    if not current_user:
        raise HTTPException(status_code=401, detail="Not authenticated")
