...
def pesos_for_seconds(seconds: int, db: DBSession, profile_id: int = _DEFAULT_PROFILE_ID) -> int:
    ...
    pesos_to_seconds(). Used to show a live peso-equivalent label on the
    minutes-mode Add Time presets in the dashboard.
    ...
    approximation for display purposes only — real coin-insert rates may
    apply volume bonuses at higher denominations that this doesn't replicate.
